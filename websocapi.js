...
function parse(htmlBody) {
    const $ = cheerio.load(htmlBody, {xmlMode: true, normalizeWhitespace: true});
    ...
    root.find('school').each(function () {
        const school = $(this);
        const schoolObj = new classes.School(school.attr("school_name"));
        schools.push(schoolObj);
        school.find('school_comment').each(function () {schoolObj.addComment($(this).text());});
        ...
        school.find('department').each(function () {
            ...
            dept.find('department_comment').each(function () {deptObj.addComment($(this).text());});
            dept.find('course_number_range_comment').each(function () {deptObj.addComment($(this).text());});
            dept.find('course_code_range_comment').each(function () {deptObj.addComment($(this).text());});

            dept.find('course').each(function () {
                const course = $(this);
                const courseObj = new classes.Course([course.attr('course_title'), course.attr('course_number')]);
                deptObj.addCourse(courseObj);
                course.find('course_comment').each(function () {courseObj.addComment($(this).text());})
                ...
                course.find('section').each(function () {
                    ...
                    const sectionData = {
                        ...
                        instructors: $('sec_instructors > instructor', section).map(function () {return $(this).text()}).get(),
                        finalExam: $('sec_final', section).text().trim(),
                        maxCapacity: $('sec_enrollment > sec_max_enroll', section).text(),
                        numCurrentlyEnrolled: $('sec_enrollment > sec_enrolled', section).text(),
                        ...
                        comments: $('sec_comment', section).text()
                    };
                    ...
                    const possibleNewOnlyNum = $('sec_enrollment > sec_new_only_reserved', section).text();
                    sectionData.numNewOnlyReserved = possibleNewOnlyNum === sectionData.classCode ? '0' : possibleNewOnlyNum;
